...
*1 active Aeon fork scanned: AITOBIAS04/CHORUS (pushed 2026-08-23, today). CHORUS skill stack unchanged at 14 skills.*
...
Skills enabled in the source (`aaronjmars/miroshark-aeon`) but not enabled in CHORUS — **5 total** (up from 4):
...
| `aeon-update` | **New in source since Aug 16.** Weekly Mon 11:00 UTC — pull upstream framework changes (canon) into this instance as a PR; 3-way merge, never clobbers operator config. Not present in CHORUS. |

**Source skill changes since 2026-08-16:**

| Change | Skill |
|--------|-------|
| ➕ Newly enabled in source | `aeon-update` (weekly Mon 11:00 UTC — sync upstream framework changes via 3-way merge PR) |

Source now enables 9 skills, up from 8. The addition is `aeon-update`, an automated framework-sync skill that keeps the instance current with upstream canon changes.

---

## Week-over-Week

Compared to the 2026-08-16 leaderboard:

**Fleet size:** Unchanged — 1 Aeon fork with readable `aeon.yml` (16th consecutive week)

**CHORUS skill count:** 14 → 14 (no change; stack stable)

**Source enabled skills:** 8 → 9 (source expanded: added `aeon-update`)

**Adoption gaps:** 4 → 5 (`aeon-update` newly added to source; token-movers, holdings, changelog, shiplog persist)
...
**MiroShark sim-tool forks (30d window):** 19 → 14 (rolling window shift; 5 forks from the previous cohort dropped below the 30-day cutoff)

**Stars / forks:** 1,430 / 298 → 1,435 / 298 (+5 stars, forks unchanged; +$161K vol day on Aug 22)

**Notification status:** SKILL_LEADERBOARD_INSUFFICIENT_DATA for the sixteenth consecutive week (1 Aeon fork, need ≥2)
...
**Layer 1 — MiroShark sim-tool forks (`aaronjmars/MiroShark`):** 14 active forks in the 30-day window (Jul 24 – Aug 23). These fork the simulation product (frontend + backend), not the Aeon agent runtime. None have `aeon.yml` — expected.
...
|------|---------|----------|----------------|-----------|
| AITOBIAS04/CHORUS | Yes | ✓ | 14 | 2026-08-23 (today) |

The source repo catalogs 200+ skills but enables only 9 by default (up from 8). CHORUS enables 14 — still the broadest stack in the fleet.

**Source skill configuration (current, as of 2026-08-23):**
...
| `aeon-update` | Mon 11:00 UTC | **New** — upstream canon sync via PR (3-way merge) |
...
- **Active MiroShark forks (pushed in last 30 days):** 14
...
- **Forks with no `aeon.yml` (sim-tool layer):** 14
