...
- GH_GLOBAL secret not set — feature skill builds PRs locally but cannot push to watched repo; 35+ consecutive blocks May 1–Jun 9 (Pre-Run Cost Estimator, Jupyter Notebook Export, Community Template Gallery, Agent Interrogation API, Simulation Impact Scorecard, One-Click Share to X, Simulation Quality Guard, Per-Round Annotation Layer, Agent Belief Heatmap, Per-Agent Trajectory Export, Simulation A/B Comparison View, Agent Persona Library, Interactive Replay Player, Inbound Launch Webhook, Coalition Detection, Private Share Links, Adversarial Stress-Test Mode, Weekly Simulation Digest, Curator Collections, Agent Journey View, Prediction-Market Calibration, MCP Simulation Tools, Simulation Tag System, Simulation Confidence Score, Agent Interaction Graph Export, Operator Dashboard, Real-Time Progress SSE, Deployment Health & Status, Multi-Metric Simulation Leaderboard, Ecosystem Registry API, French Locale, Agent Archetype Atlas, Cross-Platform Sentiment Divergence stuck as local commits; Per-Round Confidence Trajectory skipped at preflight Jun 8 and Jun 9)
