...
2. **Pre-flight: verify push access** before doing any expensive work:
   ```bash
   gh api repos/owner/repo --jq '.permissions.push // false'
   ```
   If the result is `false`, log "FEATURE_SKIP: no push access to owner/repo (GH_GLOBAL secret not set)" to `memory/logs/${today}.md` and **stop here — do NOT pick a feature, clone the repo, or send any notification.** This avoids wasting an expensive Claude run when the token lacks cross-repo push permissions.
...
4. **Check for existing open PRs** on the watched repo to avoid building something that's already pending:
   ...
   ```
   Compare your chosen idea against the open PR titles and descriptions. If an open PR already covers the same feature (even partially), **skip that idea and pick the next best one from step 3**. If ALL candidate ideas overlap with open PRs, log "FEATURE_SKIP: all candidates have open PRs" and stop.
